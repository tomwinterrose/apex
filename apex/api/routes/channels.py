...
from apex.consumers.generation_status import is_in_progress
...
from apex.database.channels.crud import mark_all_channels_deleted
from apex.database.channels.streams import (
    get_channel_streams,
    get_stream_match_details,
    refresh_stream_stats,
)
from apex.database.groups import get_group_names_by_ids
from apex.database.settings import get_dispatcharr_settings
from apex.dispatcharr import ChannelManager, get_dispatcharr_client
from apex.services import create_channel_service, create_default_service
from apex.services.stream_ordering import get_stream_ordering_service
...
def list_managed_channels(
    ...
    Returns channels tracked by Apex for lifecycle management.
...
def get_reconciliation_status():
    ...
    return ReconciliationResponse(
        ...
        summary=ReconciliationSummary(
            orphan_apex=result.summary.orphan_apex,
...
def get_channel_history(
    ...
    from apex.database.channels import get_channel_history, get_managed_channel
...
def delete_dispatcharr_channel(channel_id: int):
    ...
    but aren't tracked by Apex. This bypasses the managed channels table.
    """
...
class ResetChannelInfo(BaseModel):
    """Info about a Apex channel in Dispatcharr."""
...
def preview_reset_channels():
    """Preview all Apex-created channels that would be deleted by reset.

    Returns all channels in Dispatcharr with apex-event-* tvg_id,
    ...
    # Find ALL apex-event channels
    apex_channels = []
    for ch in all_channels:
        tvg_id = ch.tvg_id or ""
        if tvg_id.startswith("apex-event-"):
            apex_channels.append(
                ResetChannelInfo(
    ...
    return ResetPreviewResponse(
        success=True,
        channel_count=len(apex_channels),
        channels=apex_channels,
    )


@router.post("/reset", response_model=ResetExecuteResponse)
def execute_reset_channels():
    """Delete ALL Apex-created channels from Dispatcharr.

    This is a destructive operation that removes all channels with
    apex-event-* tvg_id. Also marks all managed_channels as deleted.
    ...
    # Find and delete ALL apex-event channels
    for ch in all_channels:
        tvg_id = ch.tvg_id or ""
        if not tvg_id.startswith("apex-event-"):
            continue
