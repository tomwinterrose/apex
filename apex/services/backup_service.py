...
# Filename prefixes recognized when listing/parsing backups. New backups are
# always written with the current "apex_" prefix; the older prefixes are
# read-only compatibility for backups made before the project's renames
# (teamarr -> vroomarr -> apex), so they stay visible/restorable instead of
# silently disappearing from the list after an upgrade.
_KNOWN_PREFIXES = ("apex_", "vroomarr_", "teamarr_")
...
class BackupService:
    ...
    Naming convention:
    - Scheduled: apex_scheduled_YYYYMMDD_HHMMSS.db
    - Manual: apex_manual_YYYYMMDD_HHMMSS.db
    """
    ...
    def _generate_filename(self, backup_type: str) -> str:
        ...
        Returns:
            Filename like 'apex_manual_20240115_143052.db'
        """
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        return f"apex_{backup_type}_{timestamp}.db"
    ...
    def _parse_backup_filename(self, filename: str) -> tuple[str, datetime] | None:
        ...
        Recognizes the current "apex_" prefix plus older prefixes ("vroomarr_",
        "teamarr_") from backups made before the project's renames, so
        pre-rebrand backups stay visible/restorable rather than silently
        ...
        """
        prefix = next((p for p in _KNOWN_PREFIXES if filename.startswith(p)), None)
        if prefix is None or not filename.endswith(".db"):
            return None

        try:
            # <prefix>TYPE_YYYYMMDD_HHMMSS.db
            parts = filename[len(prefix) : -3].split("_")  # Remove prefix and '.db'
            if len(parts) < 3:
                return None
    ...
    def list_backups(self) -> list[BackupInfo]:
        ...
        backups = []
        for pattern in (f"{p}*.db" for p in _KNOWN_PREFIXES):
    ...
    def restore_backup(self, filename: str) -> tuple[bool, str, str | None]:
        ...
        if db_path.exists():
            self._ensure_backup_dir()
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            pre_restore_path = self._backup_path / f"apex_pre_restore_{timestamp}.db"
            src = sqlite3.connect(str(db_path))
