...
(epic ``apexv2-eqz``). Custom leagues are TSDB-only and the whole feature is
...
from apex.database.groups import get_enabled_soccer_leagues
...
from apex.database.settings.read import get_tsdb_api_key
from apex.database.subscription import get_subscription, update_subscription
from apex.providers.tsdb import TSDBClient
...
# Maps TSDB ``strSport`` values to Apex sport codes. Used to cross-check the
...
_TSDB_SPORT_TO_APEX: dict[str, str] = {
    "soccer": "soccer",
    "cricket": "cricket",
    "rugby": "rugby",
    "boxing": "boxing",
...
def tsdb_sport_to_apex(tsdb_str_sport: str | None) -> str | None:
    """Map a TSDB ``strSport`` to a Apex sport code (None if unmapped)."""
    if not tsdb_str_sport:
        return None
    return _TSDB_SPORT_TO_APEX.get(tsdb_str_sport.strip().lower())
...
def validate_tsdb_sport_matches(tsdb_str_sport: str | None, chosen_sport: str) -> None:
    ...
    """
    mapped = tsdb_sport_to_apex(tsdb_str_sport)
    if mapped is None:
        raise CustomLeagueValidationError(
            f"TheSportsDB reports sport '{tsdb_str_sport}', which Apex does "
...
# Apex bug. This validator hits TSDB live BEFORE save: it confirms the id
