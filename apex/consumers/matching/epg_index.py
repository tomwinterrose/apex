"""Per-run EPG program index for stream-to-event matching (epic apexv2-183).
...
and categories is the matcher's job (apexv2-183.4).
"""
...
class EPGProgramIndex:
    ...
    def build(
        ...
        Args:
            ...
            exclude_apex: Drop programs from our own generated EPG source.
            own_source_name: The CONFIGURED name of our own EPG source in
                ...
                ``is_apex`` check (``epg_source == "_Apex"``), which
                silently never matches installs whose source isn't literally
                named that (e.g. the default "Apex", no underscore).
        ...
        for stream_tvg, program_tvg in tvg_id_resolution.items():
            ...
            if exclude_apex:
                programs = [
                    p for p in programs
                    if not p.is_apex and not (own_source_name and p.epg_source == own_source_name)
                ]
            if not programs:
                continue
