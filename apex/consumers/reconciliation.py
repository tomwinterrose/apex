...
Detects and resolves inconsistencies between Apex's managed_channels
database and Dispatcharr's actual channel state.

Issue Types:
- Orphan (Apex): Record exists in DB but channel missing in Dispatcharr
- Orphan (Dispatcharr): Channel with apex-* tvg_id exists but no DB record
- Duplicate: Multiple channels for the same event
- Drift: Channel settings differ between Apex and Dispatcharr
...
from apex.dispatcharr import ChannelManager
from apex.dispatcharr.factory import DispatcharrConnection, get_dispatcharr_connection
...
class ReconciliationIssue:
    ...
    issue_type: str  # orphan_apex, orphan_dispatcharr, duplicate, drift
...
class ChannelReconciler:
    """Reconciles Apex managed channels with Dispatcharr.
    ...
    def reconcile(
        ...
        try:
            with self._db_factory() as conn:
                # Step 1: Detect orphans (Apex records without Dispatcharr channels)
                apex_orphans = self._detect_orphan_apex(conn)
                result.issues_found.extend(apex_orphans)

                # Step 2: Detect orphans (Dispatcharr channels without Apex records)
    ...
    def _detect_orphan_apex(
        self,
        conn: Connection,
    ) -> list[ReconciliationIssue]:
        """Detect Apex records that have no corresponding Dispatcharr channel.
        ...
        if issues:
            logger.info("[ORPHAN_APEX] Found %d orphan(s)", len(issues))
    ...
    def _detect_orphan_dispatcharr(
        ...
        """Detect Dispatcharr channels with apex-* tvg_id that aren't tracked.

        These are channels that may have been created manually or where
        Apex's database record was lost.
        """
        issues = []
        ...
        for channel in all_channels:
            ...
            if has_apex_tvg_id:
                event_id = tvg_id.replace("apex-event-", "")
    ...
    def _detect_drift(
        ...
        """Detect channels where Apex's expected state differs from Dispatcharr.
    ...
    def _apply_fixes(
        ...
        for issue in result.issues_found:
            ...
            try:
                if issue.issue_type == "orphan_apex":
                    # Mark as deleted in Apex DB
    ...
    def verify_channel(
        ...
        """
        from apex.database.channels import get_managed_channel
