...
class ChannelCleanup(_LifecycleHost):
    ...
    def _recalculate_deletion_times(self, conn) -> int:
        ...
        from apex.database.channels import get_all_managed_channels, update_managed_channel
    ...
    def cleanup_deleted_streams(
        ...
        """
        from apex.consumers.stream_match_cache import compute_fingerprint
        from apex.database.channels import (
            get_channel_streams,
    ...
    def cleanup_orphan_dispatcharr_channels(self) -> dict:
        ...
        Orphan channels are Dispatcharr channels with apex-event-* tvg_id
        ...
        try:
            with self._db_factory() as conn:
                # Get all apex channels from Dispatcharr
                with self._dispatcharr_lock:
                    all_dispatcharr = self._channel_manager.get_channels()

                apex_channels = [
                    c for c in all_dispatcharr
                    if (c.tvg_id or "").startswith("apex-event-")
                ]

                if not apex_channels:
                    return result
    ...
    def cleanup_disabled_groups(self) -> dict:
        ...
        channel still fed by other enabled groups (apexv2-5xou).
