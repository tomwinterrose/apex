...
1. Generate consistent tvg_id: apex-event-{event_id}
...
from apex.dispatcharr import ChannelManager, EPGManager, LogoManager
...
def compute_external_occupied(
    ...
    Compares the Dispatcharr channel cache against Apex's managed_channels
    ...
    # Get all channel numbers Apex manages
    apex_numbers: set[int] = set()
    with db_factory() as conn:
        ...
        for row in rows:
            try:
                apex_numbers.add(int(float(row["channel_number"])))
            except (ValueError, TypeError):
                pass

    external = dispatcharr_numbers - apex_numbers
...
def create_lifecycle_service(
    ...
    """
    from apex.database.channels import get_dispatcharr_settings
