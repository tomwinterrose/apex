...
CREATE TABLE IF NOT EXISTS settings (
    ...
    -- EPG program-data matching master switch (epic apexv2-183.6). Default
    ...
    -- XC (Xtream) provider EPG fallback (epic apexv2-crs). Default OFF
    ...
    -- backup, when a stream's M3U account is an Xtream panel, Apex can fetch
    ...
    -- EPG channel-source mode (epic apexv2-183.9). When enabled, an additional
    ...
    -- channel's own EPG), alongside the per-group M3U-group EPG matching. Apex's
    ...
    -- stream-ordering rule. (epic apexv2-ybt.2)
    epg_channel_source_groups TEXT DEFAULT '[]',

    -- EPG stream time-windowing buffers (epic apexv2-183.5).
    ...
    -- XMLTV
    xmltv_generator_name TEXT DEFAULT 'Apex',
    xmltv_generator_url TEXT DEFAULT 'https://github.com/tomwinterrose/apex',
    ...
    dispatcharr_epg_id INTEGER,               -- Apex's EPG source ID in Dispatcharr
    ...
    auto_fix_orphan_dispatcharr BOOLEAN DEFAULT 1, -- Auto-delete channels in Dispatcharr not tracked by Apex
    ...
    -- When enabled, purges all Apex channels from Dispatcharr on the specified schedule
    ...
    update_github_repo TEXT DEFAULT 'apex',           -- GitHub repo name (for forks)
...
-- Seed sports with proper display names (motorsports-only: Apex)
...
CREATE TABLE IF NOT EXISTS leagues (
    ...
    -- Custom League Flag
    -- 1: user-added via the UI (TSDB-only, premium-gated; see epic apexv2-eqz).
...
INSERT OR REPLACE INTO leagues (league_code, provider, provider_league_id, provider_league_name, display_name, sport, logo_url, logo_url_dark, import_enabled, league_alias, league_id, event_type, gracenote_category, fallback_provider, fallback_league_id, tsdb_tier, enabled) VALUES
    ...
    -- per-event-per-session season data (apex/providers/tsdb/racing.py).
...
CREATE TABLE IF NOT EXISTS managed_channel_streams (
    ...
    -- Time-windowed membership (epic apexv2-183.5).
